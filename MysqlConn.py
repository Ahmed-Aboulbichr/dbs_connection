...
class MysqlConn:
    def __init__(self):
        d = rc.read_ini("config_mysql.ini")
        self.conn = mc.connect(host=d["host"], user=d["user"], password=d["password"], database=d["database"])
    ...
    def insert(self, cne, cni, nom, prenom, date, mail):
        ...
        try:
            with self.conn.cursor() as cur:
                cur.execute("INSERT INTO etudiant(cne, cni, nom, prenom, date_naissance, mail_academique) VALUES (%s, %s, %s, %s, %s, %s)",
